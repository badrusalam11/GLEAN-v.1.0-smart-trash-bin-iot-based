...
void loop() {
 ...
 String url1 = (String)"http://"+host+(String)"/skripsi/template/dashboard/back1.php";
    ...
    if (httpCode1 > 0) {
      ...
      //Parsing
      const size_t bufferSize1 = JSON_OBJECT_SIZE(1);
      DynamicJsonBuffer jsonBuffer1(bufferSize1);
      JsonObject& root1 = jsonBuffer1.parseObject(http1.getString());
      // Parameters
      const char* kode_valid1 = root1["kode_valid1"]; // "kode valid"
      // Output to serial monitor
      Serial.print("kode t. sampah valid :");
      Serial.println(kode_valid1);
      String kode_valid_string = String(kode_valid1);
      lcd.begin();
      lcd.home();
      lcd.print("Kode : ");
      lcd.setCursor(0,1);
 ...
 String url = (String)"http://"+host+(String)"/skripsi/template/dashboard/back.php";
  ...
  if (berat<0){
    berat=0;
      ...
      // Parameters
      const char* username = root["username1"]; // "username"
      const char* kode = root["kode1"]; // "kode"
      const int acak = root["acak1"]; //kode acak
      const char* servo = root["servo1"]; // "servo"
      const int berat_db = root["berat1"]; // "servo"
      // Output to serial monitor
